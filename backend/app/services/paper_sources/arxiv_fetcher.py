...
class ArXivFetcher(PaperFetcher):
    ...
    def _fetch_papers_sync(
        ...
        if categories and len(categories) > 0:
            ...
            # When no categories specified, search all of arXiv
        ...
        while attempt < retry_attempts:
            try:
                ...
                logger.error(f"Error fetching papers from arXiv (attempt {attempt}/{retry_attempts}): {str(e)}")
                if attempt < retry_attempts:
                    logger.info(f"Retrying in {wait_time} seconds...")
                    time.sleep(wait_time)
                else:
                    logger.error("Max retry attempts reached. Could not fetch papers from arXiv.")
                    return []
